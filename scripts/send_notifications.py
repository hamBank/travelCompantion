...
def main() -> None:
    with Session(engine) as session:
        ...
        # Webhook subscription reconciliation (plan-14) deliberately does NOT
        # run here — it's on its own, much coarser cron
        # (scripts/reconcile_flight_alerts.py) since it makes at least two
        # AeroDataBox calls every time it runs regardless of activity, which
        # this script's 15-minute cadence would blow through the 600/month
        # free-tier budget on alone. Live flight polling below still runs as
        # the per-flight fallback for any flight whose webhook subscription
        # couldn't be created (or hasn't been reconciled yet).
        #
        # Needs AeroDataBox configured — skip quietly on servers that don't
        # have it set, rather than erroring the cron.
