...
def _daily(times, tmax, tmin, codes):
    return {"daily": {"time": times, "temperature_2m_max": tmax,
                      "temperature_2m_min": tmin, "weathercode": codes}}


def test_parse_daily_skips_null_rows():
    payload = _daily(
        ["2026-07-22", "2026-07-23"], [30, None], [20, 18], [1, 61],
    )
    out = weather.parse_daily(payload)
    assert out == {"2026-07-22": {"tmin": 20, "tmax": 30, "code": 1}}


def test_average_climatology_means_temps_and_modal_code():
    y1 = _daily(["2023-07-22"], [30], [20], [1])
    y2 = _daily(["2024-07-22"], [32], [22], [1])
    y3 = _daily(["2025-07-22"], [28], [18], [61])
...
def test_get_weather_uses_forecast_within_horizon():
    ...
    def fake_fetch(url):
        ...
        return _daily(["2026-07-01", "2026-07-02"], [25, 26], [15, 16], [0, 2])
